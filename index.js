var finances = [
  // month and profit loss for the month //
  ["Jan-2010", 867884],
  ["Feb-2010", 984655],
  ["Mar-2010", 322013],
  ["Apr-2010", -69417],
...
var avg = Math.round((total / finances.length) * 100) / 100;

console.log("net-total amount", total);
console.log("avg amount", avg);
// total profit loss over entire period is sum of all profit loss //

for (var p = 0; p < finances.length; p++) {
  for (var j = 0; j < finances[p].length; j++) {
    if (typeof finances[p][j] !== "string") {
        change = finances[p][j] - net;
        net = finances[p][j];

      if (change > greatest[1]) {
        greatest =[finances[p][0],finances[p][1]] ;
      }
      if (change < least[1]) {
        least =[finances[p][0],finances[p][1]] ;
      }
    }
  }
}
